...
class Expression_bedgraph:
    ...
    def induced_genes(self,expression_file,gtf):
        ...
        refgene_data_txn_length['tss'] = refgene_data.groupby(['name2']).cumcount()+1#add tss number

        #LEAST DISTANT GENES IN THE GENOME
        clean_refgene_data = refgene_data_txn_length[refgene_data_txn_length.tss == 1]#extract genes with tss 1
        data = clean_refgene_data[['chrom','txStart','name2']].copy()
        ...
        all_distance_combination_genes.to_csv("output/tf_analysis/genome.all_distance_combination_genes.distance", sep='\t',index=False)
        gene_least_distant_table.to_csv("output/tf_analysis/genome.least_distant_genes.distance", sep='\t',index=False)
        print("  Files generated: \n\toutput/tf_analysis/"+self.gtf[:-11]+"least_distant_genes.distance\n\toutput/tf_analysis/"+self.gtf[:-11]+"all_combination_distant_genes.distance\n")
...
class Inducedgenesclustering:
    ...
    def induced_genes_clusteringByDistance(self):
        ...
        pybedtools.bedtool.BedTool.cluster(pybedtools.bedtool.BedTool('output/tf_analysis/induced_genes_5p_3p_coordinates.bed'),d=induced_genes_cluster_distance).saveas('output/tf_analysis/induced_genes_bed_tool_clust.bed')

        final_cluster = pd.read_csv('output/tf_analysis/induced_genes_bed_tool_clust.bed',sep="\t",header=None,names=['chr','start1','start2','end','gene','clust1'])
        ...
        final_cluster.to_csv('output/tf_analysis/induced_genes_final_cluster.bed',sep="\t",header=None,index=False)
...
class Random_selection_least_distant_genes:
    def __init__(self):
        '''Read File'''
    def random_selection_genes(self,least_distant_genes,induced_gene_file):
        print("##### Random selection of genes from clean RefSeq GTF file ...")
        self.least_distant_genes = least_distant_genes
        self.no_of_induced_genes = induced_gene_file

        #RANDOMLY SELECTED LEAST DISTANT GENES
        no_of_induced_genes = sum(1 for line in open("input_data/"+self.no_of_induced_genes))-1
        genes_distance = pd.read_csv('output/tf_analysis/'+self.least_distant_genes, sep="\t", header=0)
        random_select_genes = genes_distance.sample(no_of_induced_genes)
        random_select_genes.to_csv('output/tf_analysis/'+str(self.least_distant_genes[:-29])+"_randomly_selected."+str(no_of_induced_genes)+"_least_distant_genes.distance", sep='\t',index=False)
        print("  Files generated: output/tf_analysis/\n\t"+str(self.least_distant_genes[:-29])+"_randomly_selected_"+str(no_of_induced_genes)+"_least_distant_genes.distance")

        #PLOT OBSERVED VS RANDOMLY SELECTED GENES
        def obs_vs_rand(obs,rand):
            obs = pd.read_csv(obs,sep="\t",header=0,usecols=['distance'])
            obs['resource'] = 'Observed'
            rand = pd.read_csv(rand,sep="\t",header=0,usecols=['distance'])
            rand['resource'] = 'Random'
            ...
            p = (ggplot(data=data)+
                    ...
                    theme_classic())
            ggsave(plot=p, filename='output/tf_analysis/observed_vs_randomly_selected_genes_distribution.png', dpi=150)
            print('Obs vs Randomly selected gene distribution comparison plot is generated:\n\toutput/tf_analysis/observed_vs_randomly_selected_genes_distribution.png')

        obs_vs_rand('output/tf_analysis/genome.least_distant_genes.distance',
                    'output/tf_analysis/'+str(self.least_distant_genes[:-29])+'_randomly_selected.'+str(no_of_induced_genes)+'_least_distant_genes.distance')
...
class Tf_peak_tss_binding:
    ...
    def tf_peak_tss_cluster(self,filename,tf,induced_gene_tss,filterscore):
        ...
        pybedtools.bedtool.BedTool.cluster(pybedtools.bedtool.BedTool('output/tf_analysis/combined_gene_5p_tf_peak_calls_data.bed'),d=promoter_region).saveas('output/tf_analysis/gene_5p_tf_peak_calls_bed_tool_clust.bed')
...
random_selection_least_distant_genes = Random_selection_least_distant_genes()
random_selection_least_distant_genes.random_selection_genes('genome.least_distant_genes.distance',rna_expression_file)
